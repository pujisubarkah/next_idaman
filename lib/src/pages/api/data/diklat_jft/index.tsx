    ...
    if (req.method === 'GET') {
        try {
            ...
            const { data: kategoriData, error: kategoriError } = await supabase
                ...
                .select(`
                    diklat_fungsional_id,
                    diklat_fungsional_nm
                `);
            ...
            const { data: riwayatDiklatData, error: riwayatDiklatError } = await supabase
                ...
                .select('diklat_fungsional_id, peg_id')
                .eq('diklat_jenis', 2);  // Filter for diklat_jenis = 2
            ...
            const pegawaiCountPerKategori = (riwayatDiklatData || []).reduce((acc, item) => {
                if (!acc[item.diklat_fungsional_id]) {
                    acc[item.diklat_fungsional_id] = 0;
                }
                acc[item.diklat_fungsional_id]++;
                return acc;
            }, {});

            // Structuring the response data
            const responseData = kategoriData.map(kategori => {
                const pegawaiCount = pegawaiCountPerKategori[kategori.diklat_fungsional_id] || 0;

                // Adding employee data related to the category
                const pegawaiInKategori = pegawaiData.filter(pegawai => {
                    return riwayatDiklatData.some(diklat => diklat.diklat_fungsional_id === kategori.diklat_fungsional_id && diklat.peg_id === pegawai.peg_id);
                });
