...
const Dashboard = () => {
  ...
  const fetchStatuses = async () => {
    setLoading(true);
    try {
      const response = await axios.get("/api/jft");
          ...
          <table className="w-full border border-teal-600 rounded-lg overflow-hidden my-5">
            <thead>
              <tr className="bg-teal-900 text-white">
                ...
                <th className="p-3 border border-teal-700 text-left font-bold uppercase text-sm">Skill</th>
            ...
            <tbody>
              {statuses.map(({ jf_nama, jf_skill, total_pegawai, pegawai }, index) => (
                <tr key={index} className={index % 2 === 0 ? "bg-teal-50" : "bg-white"}>
                  ...
                  <td className="px-4 py-2 border border-teal-300">{jf_skill}</td>
                  <td
