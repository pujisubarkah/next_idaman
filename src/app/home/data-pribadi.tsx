...
const DataPribadi = () => {
    ...
    interface DataEntry {
        label: string;
        value: string;
        buttons?: { label: string; icon: JSX.Element }[];
        label2?: string;
        value2?: string;
        label3?: string;
        value3?: string;
    ...
    const dataEntries: DataEntry[] = [
        ...
        { label: "Status Pegawai", value2: datapribadi.alamatRumah, label2: "TMT CPNS", value: datapribadi.rt, label3: "TMT PNS", value3: datapribadi.alamatRumah },
        { label: "Jenis ASN", value: datapribadi.Agama },
        { label: "Jenis PNS", value: datapribadi.jenisPNS },
        { label: "Pendidikan Awal", value: datapribadi.pendidikanAwal, label2: "Tahun Pendidikan Awal", value2: datapribadi.Agama },
        { label: "Pendidikan Akhir", value: datapribadi.pendidikanAkhir, label2: "Tahun Pendidikan Akhir", value2: datapribadi.Agama },
        ...
        { label: "No KTP", value: datapribadi.noKTP, label2: "No. NPWP", value2: datapribadi.Agama },
        ...
        { label: "Kab/Kota", value: datapribadi.kabKota, label2: "Kode Pos", value2: datapribadi.Agama },
        ...
        { label: "Kab/Kota", value: datapribadi.kabKotaKTP, label2: "Kode Pos", value2: datapribadi.Agama },
            ...
            <tbody>
                {dataEntries.map((entry, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-teal-50" : "bg-white"}>
                        ...
                        {/* Value 1 with Optional buttons below */}
                        <td className="px-4 py-2 w-2/6">
                            <div>{entry.value || " "}</div>
                            {entry.buttons && entry.buttons.length > 0 && (
                                <div className="mt-2 flex space-x-2">
                                    {entry.buttons.map((btn, i) => (
                                        <button key={i} className="px-4 py-1 bg-teal-500 text-white rounded hover:bg-blue-600 flex items-center">
                                            {btn.icon}
                        ...
                        {/* Label 2 */}
                        <td className="px-4 py-2 font-semibold w-1/20">{entry.label2}</td>
    
                        {/* Value 2 */}
                        <td className="px-4 py-2 w-1/10">
                            <div>{entry.value2 || " "}</div>
                            {/* Only show buttons for Value 2 if they exist */}
                            {entry.buttons && entry.buttons.length > 0 && entry.value2 && (
                                <div className="mt-2 flex space-x-2">
                                    {entry.buttons.map((btn, i) => (
                                        <button key={i} className="px-4 py-1 bg-teal-500 text-white rounded hover:bg-blue-600 flex items-center">
                                            {btn.icon}
                        ...
                        {/* Label 3 */}
                        <td className="px-4 py-2 font-semibold w-1/20">{entry.label3}</td>
    
                        {/* Value 3 */}
                        <td className="px-4 py-2 w-1/10">
                            <div>{entry.value3 || " "}</div>
                            {/* Only show buttons for Value 3 if they exist */}
                            {entry.buttons && entry.buttons.length > 0 && entry.value3 && (
                                <div className="mt-2 flex space-x-2">
                                    {entry.buttons.map((btn, i) => (
                                        <button key={i} className="px-4 py-1 bg-teal-500 text-white rounded hover:bg-blue-600 flex items-center">
                                            {btn.icon}
