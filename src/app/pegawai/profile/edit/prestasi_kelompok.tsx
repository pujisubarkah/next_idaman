...
const PrestasiKelompok: React.FC<PrestasiKelompokProps> = ({ nip }) => {
  ...
  const fetchRiwayatPrestasiKelompok = useCallback(async (nip: string) => {
    try {
      const response = await axios.get(`/api/riwayat/prestasi_kelompok?peg_id=${nip}`);
      const mappedData: RiwayatData[] = response.data.map((item: any) => ({
        id: item.prestasi_kelompok_id,
  ...
  }, [nip, fetchRiwayatPrestasiKelompok]);
  ...
  const handleAddPrestasi = async () => {
    ...
    try {
      ...
      const response = await axios.post(`/api/riwayat/prestasi_kelompok`, dataToSend);
      ...
      setKelompokData([...kelompokData, newEntry]);
      resetForm();
    } catch (error) {
  ...
  const handleEditPrestasi = (id: number) => {
    const entryToEdit = kelompokData.find(item => item.id === id);
    if (entryToEdit) {
  ...
  const handleUpdatePrestasi = async () => {
    ...
    try {
      ...
      const response = await axios.put(`/api/riwayat/prestasi_kelompok/${formData.id}`, dataToSend);
      ...
      setKelompokData(kelompokData.map(item => (item.id === formData.id ? updatedEntry : item)));
      resetForm();
    } catch (error) {
  ...
  const handleDeletePrestasi = async (id: number) => {
    try {
      await axios.delete(`/api/riwayat/prestasi_kelompok/${id}`);
      setKelompokData(kelompokData.filter(item => item.id !== id));
    } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white w-3/4 md:w-1/2 p-6 rounded shadow-lg">
            <h4 className="text-lg font-semibold mb-4">
              {formData.id ? "Edit" : "Tambah"} Prestasi Kelompok
            </h4>
...
export default PrestasiKelompok;
