    ...
    return data.map((row, index) => {
      ...
      return (
        <tr key={index} className={index % 2 === 0 ? 'bg-teal-50' : 'bg-white'}>
          ...
          <td className="p-3 border border-teal-300">{row.satuan_kerja_nama}</td>
          {golonganColumns.map((golongan) => (
            <td 
              key={golongan} 
              className="p-3 border border-teal-300 text-center cursor-pointer"
          ...
          <td 
            className="p-3 border border-teal-300 text-center" 
            ...
            <table className="w-full border border-teal-600 rounded-lg">
              <thead>
                <tr className="bg-teal-900 text-white">
                  ...
                  <th className="p-3 border border-teal-700 text-center font-bold uppercase text-sm" colSpan={golonganColumns.length}>
                    Golongan
                  </th>
                  <th className="p-3 border border-teal-700 text-left font-bold uppercase text-sm" rowSpan={2}>
                    Belum Terdata
                ...
                <tr className="bg-teal-900 text-white">
                  {golonganColumns.map((golongan) => (
                    <th key={golongan} className="p-2 border border-teal-700 text-center">
                      {golongan}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                ...
                <tr className="bg-teal-200">
                  ...
                  {totals.map((total, index) => (
                    <td key={index} className="p-3 border border-teal-300 text-center font-bold">{total}</td>
                  ))}
                  <td className="p-3 border border-teal-300 text-center">-</td>
                  <td className="p-3 border border-teal-300 text-center">-</td>
                </tr>
              </tbody>
            </table>
